...
Util.cipher = function (phrase) {
  ...
  for(var i = 0;i<phrase.length;i++){
    asciCode=phrase.charCodeAt(i);
    if((asciCode>=65 && asciCode<=89) || (asciCode>=97 && asciCode<=121)){
      newPhrase+=String.fromCharCode(asciCode+1);
    }
    else if(asciCode==90 || asciCode==122){
      newPhrase+=String.fromCharCode(asciCode-25);
    }
    else{
      newPhrase+=String.fromCharCode(asciCode);
    }
  }
  return newPhrase;
};
